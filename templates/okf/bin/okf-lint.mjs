...
function usage() {
  return "Usage: okf-lint.mjs --root <physical-git-root> [--profile <root-relative-json>] [--mode legacy|warning|strict-new|strict-bundle|strict] [--strict-path <knowledge/.../record.md> ...] [--format text|json] [--fail-on error|none]";
}
...
for (let index = 0; index < args.length; index += 1) {
  ...
  if (!["--root", "--profile", "--mode", "--strict-path", "--format", "--fail-on"].includes(flag) || index + 1 >= args.length) {
    console.error(usage());
    process.exit(2);
  }
...
try {
  ...
  const result = lintBundle(rootInfo.root, { mode: selectedMode, strictPaths, extensions: profile.extensions });
