...
function usage() {
  return "Usage: okf-run-plan.mjs --check-only --root <repository-root> --revision <full-git-sha> --operator-request <text> --max-items <n> --max-input-bytes <n> --max-generated-bytes <n> --max-runtime-seconds <n> [--format text|json]";
}
...
for (let index = 0; index < args.length; index += 1) {
  ...
  if (!["--root", "--revision", "--operator-request", "--max-items", "--max-input-bytes", "--max-generated-bytes", "--max-runtime-seconds", "--format"].includes(flag) || index + 1 >= args.length) {
    console.error(usage()); process.exit(2);
  }
  values[flag] = args[++index];
}
if (!checkOnly || !values["--root"] || !values["--revision"] || !values["--operator-request"] || !values["--max-items"] || !values["--max-input-bytes"] || !values["--max-generated-bytes"] || !values["--max-runtime-seconds"] || ![undefined, "text", "json"].includes(values["--format"])) {
  console.error(usage()); process.exit(2);
}
...
try {
  const plan = validateRunPlan({
    trigger: "operator", operator_request: values["--operator-request"], root: values["--root"], revision: values["--revision"], inbox_path: "knowledge/inbox", max_sessions: 1,
