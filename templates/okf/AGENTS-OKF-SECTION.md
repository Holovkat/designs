...
Run curation only through the explicit repository-scoped bounded manual workflow. Reversible archive/compaction requires a reviewed manifest and rollback proof; permanent deletion requires separate path-specific approval.
...
- The installer distributes no scheduled-curation adapter, prompt/profile, or dispatcher. No OKF command autonomously edits AGENTS.md, traverses a home/parent workspace, schedules curation, or mutates another repository.
