...
- [x] 09 — Model Providers & Selection … `corrected-documentation/09-MODEL-PROVIDERS-AND-SELECTION.md`
  ...
  - 2025-10-23: Added `sanitize_tool_overrides` guard so plan/apply_patch/web-search/view-image (and their feature toggles) disable themselves whenever a provider cannot execute tool calls (Z.AI Chat endpoints, OSS models without tool support). Confirmed via `cargo test -p codex-agentic-core` and manual CLI/TUI runs swapping between `gpt-5-codex` and `zai`.
...
- [x] 16 — Custom Providers & BYOK Modal … `codex-pro/designs/codex-pro/functional-design/16-CUSTOM-PROVIDERS-BYOK.md`
  - 2025-10-12: Implemented `/BYOK` flow with base URL normalization, Chat/Responses auto-detection, cached model placeholders, and shared provider merge for CLI/TUI (`codex-rs/codex-agentic-core`, `codex-rs/tui`, `codex-rs/exec`, `codex-rs/core`). Verified via `cargo check`, `cargo run --bin codex-agentic -- --help`, and `cargo run --bin codex-agentic -- exec --model glm-4.6 -- "hi"` (now automatically falls back to Chat Completions with the minimal Coding Plan prompt).
  - 2025-10-15: Rebased the CLI stream tests on isolated BYOK fixtures so `cargo test -p codex-core` covers Coding Plan behaviour end-to-end; documented the prompt swap and `/chat/completions` fallback.
  - 2025-10-23: Added provider-kind dropdown, reasoning controls, manual “Refresh models” workflow, cached-model viewer, and `extra_headers` support (applied to discovery + runtime requests). Validated against Z.AI (`tenant=83005`) via `cargo test -p codex-agentic-core`, `cargo test -p codex-tui`, and `cargo run --bin codex-agentic -- exec --model glm-4.6 -- "hi"` (returns 200 instead of 401/400). Documentation updated with manual refresh guidance and provider-kind semantics.
- [x] 22 — Unified Provider Reasoning Handling … `codex-pro/designs/codex-pro/functional-design/22-UNIFIED-PROVIDER-REASONING-HANDLING.md`
  - 2025-10-23: Introduced provider-kind aware resolver, `<think>` parser, tool gating for Ollama, Anthropic thinking-budget payloads, reasoning transcript styling, and BYOK migrations. Tests: `cargo test -p codex-agentic-core`, `cargo test -p codex-tui`, and manual CLI/TUI turns across OpenAI/Ollama/Anthropic providers (known outstanding: `suite::cli_stream::integration_creates_and_checks_session_file`).
- [x] 23 — Memory Manager & MiniCPM Context Store … `codex-pro/designs/codex-pro/functional-design/23-MEMORY-MANAGER-IMPLEMENTATION.md`
  - 2025-10-27: Landed end-to-end memory pipeline (`core/src/memory/*`), MiniCPM download/cache integration, planner preview events, `/memory` TUI overlay, and `codex memory` CLI admin surface. Added status/diagnostics wiring plus scroll indicators, refreshed docs, and expanded tests (`cargo test -p codex-core memory::retriever::tests`, `cargo test -p codex-tui`, `cargo test -p codex-cli memory_cli_crud_flow`).
